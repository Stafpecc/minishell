...
/*   Updated: 2025/07/24 18:28:24 by ldevoude         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int	enoent_cases(t_command_exec *node, int execve_failed)
{
	if (errno == ENOENT && execve_failed == 0)
	{
		join_err_msg_and_write("minishell: ", node->cmd_parts[0],
			": command not found\n");
	}
	else if (execve_failed == 1)
	{
		join_err_msg_and_write("minishell: ", node->cmd_parts[0],
