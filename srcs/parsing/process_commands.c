...
/*   Updated: 2025/05/08 14:13:37 by stafpec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void process_quotes(t_token *tokens, t_command *curr)
{
	if ((tokens)->type == TOK_SINGLE_QUOTES || (tokens)->type == TOK_DOUBLE_QUOTES)
    {
        curr->in_simple_quote = true;
        curr->in_double_quote = false;
    }
    else
    {
        curr->in_simple_quote = false;
        curr->in_double_quote = true;
    }
}
